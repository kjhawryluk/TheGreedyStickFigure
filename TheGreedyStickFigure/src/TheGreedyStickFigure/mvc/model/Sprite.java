...
import static TheGreedyStickFigure.mvc.model.Movable_Person.MAX_LEFT;
import static TheGreedyStickFigure.mvc.model.Movable_Person.MAX_RIGHT;
...
//radial coordinates
    ...
    /**
     * The constant SHIFT_SPEED.
     */
    public static final int SHIFT_SPEED = 20;
    ...
    public void move() {
        double maxLeft = 0;
        double maxRight = Game.DIM.getWidth();
        Point pnt = getCenter();
        ...
        if (dX - getRadius() < maxLeft) {
            dX = maxLeft + getRadius();
            setDeltaX(-getDeltaX());
        } else if (dX + getRadius() > maxRight) {
            dX = maxRight - getRadius();
            setDeltaX(-getDeltaX());
        }
